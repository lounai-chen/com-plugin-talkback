 ...
 * 广播接受者
 */
public class PttWidgetTimeProvider extends AppWidgetProvider {
    ...
    public static void updateTime(Context context) {
        if (timeDis != null && !timeDis.isDisposed()) {
            timeDis.dispose();
            timeDis = null;
        }

        timeDis = Observable.interval(1, 30, TimeUnit.SECONDS).subscribe(it -> {
            RemoteViews remoteViews = new RemoteViews(context.getPackageName(), LAYOUT_RES_ID);
    ...
    private final static int LAYOUT_RES_ID = R.layout.layout_widget_ptt_time;
    ...
    private static void setClick(Context context, RemoteViews remoteViews) {
        ...
        Intent btIntent = new Intent(context, PttWidgetTimeProvider.class);  // setClass 这个必须要设置，不然点击效果会无效
        btIntent.setAction(OPEN_CHANNEL);
        PendingIntent btPendingIntent = PendingIntent.getBroadcast(context, 0, btIntent, PendingIntent.FLAG_UPDATE_CURRENT);
...
//            openChannelList(context);
    ...
    public static void updatePtt(Context context, String userName, int userId, String channelName, int channelId, boolean self) {
        ...
        if (!TextUtils.isEmpty(userName)) {
            ...
            remoteViews.setViewVisibility(R.id.tv_uid, View.GONE);
            //remoteViews.setTextViewText(R.id.tv_uid, "用户ID:" + userId);
            remoteViews.setTextViewText(R.id.tv_uname, "用户:" + userName);
...
//    }
    ...
    private static void refreshWidget(Context context, RemoteViews remoteViews) {
        ...
        ComponentName componentName = new ComponentName(context, PttWidgetTimeProvider.class);
